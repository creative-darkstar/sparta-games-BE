...
@api_view(['POST'])
def game_playtime_enter(request, game_pk):
    # 로그인 여부 확인
...
def game_playtime_exit(request, game_pk):
    ...
    playtimeend.end_time = timezone.now()  # 현재 시간으로 end_time
    playtimeend.total_playtime = (playtimeend.end_time - playtimeend.start_time).total_seconds()  # playtime_seconds로 playtime_seconds 계산
    playtimeend.save()
